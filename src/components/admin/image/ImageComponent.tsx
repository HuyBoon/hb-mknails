...
interface ImageComponentProps {
    ...
    maxFiles?: number;
    category: string;
    registerImageComponent?: (selectedFiles: File[], handleUpload: () => Promise<void>) => void;
}
...
const ImageComponent: React.FC<ImageComponentProps> = ({
    ...
    // Expose selectedFiles and handleUpload to parent
    React.useEffect(() => {
        if (registerImageComponent) {
            registerImageComponent(selectedFiles, handleUpload);
        }
    }, [selectedFiles, registerImageComponent]);
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (result.success && result.images && result.images.length > 0) {
                ...
                throw new Error("Không có ảnh nào được tải lên hoặc dữ liệu không hợp lệ.");
            }
        } catch (error: any) {
